...
`OnlineSessionHost` owns a `BattleSettlementCoordinator` beside login and archive services. It accepts one terminal result per `run_id`, sends `CombatResultReq`, merges the returned `PlayerArchive`, saves it through a settlement-dedicated `ArchiveSessionService`, and reports completion only after `SaveArchiveResp.success`. The dedicated archive operation channel keeps a retryable settlement save failure from failing the login/session coordinator. If the connection generation changes while settlement is pending, the coordinator resends the same `run_id`; the backend idempotency record prevents duplicate rewards.
