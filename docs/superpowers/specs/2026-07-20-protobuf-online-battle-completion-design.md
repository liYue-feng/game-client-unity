...
`OnlineSessionHost` owns a `BattleSettlementCoordinator` beside login and archive services. It accepts one terminal result per `run_id`, sends `CombatResultReq`, merges the returned `PlayerArchive`, saves it through `ArchiveSessionService`, and reports completion only after `SaveArchiveResp.success`. If the connection generation changes while settlement is pending, the coordinator resends the same `run_id`; the backend idempotency record prevents duplicate rewards.
